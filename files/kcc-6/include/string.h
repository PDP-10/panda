...
char *strdup(const char *);			/* new <- old */
...
int strcasecmp(const char*, const char*);	/* s1, s2 */
int strcoll(const char*, const char*);		/* s1, s2 */
int strncmp(const char*, const char*, size_t);	/* s1, s2, n */
/* Remap ambiguous external */
#pragma private define strncasecmp strncsecmp
int strncasecmp(const char*, const char*, size_t);/* s1, s2, n */
...
extern	char *strcat(), *strchr(), *index(), *strcpy(), *strncat(), *strncpy(),
	...
	*strerror(), *strdup();
extern int
	strcmp(), strncmp(), strpos(), strrpos(), bcmp();
