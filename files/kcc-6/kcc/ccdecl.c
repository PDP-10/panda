...
static NODE *funcdef P_((SYMBOL *b,SYMBOL *d,SYMBOL *syment));
static int nsetjmps P_((void));
static void plcmpare P_((TYPE *t1,TYPE *t2,int flag));
static void plcmpold P_((SYMBOL *s,TYPE *nt,TYPE *ot));
static void plcheck P_((TYPE *t));
static void prttok P_((int i));
static void prtindent P_((int in));
static void prtsimple P_((TYPE *t,int *needwsp));
static void prtlpaf P_((TYPE *t,int *needwsp));
static void prtrpaf P_((SYMBOL *sp,TYPE *t,TYPE *tp,int in));
static void prtspec P_((SYMBOL *s,SYMBOL *sp,TYPE *t,TYPE *tp,int in));
static void prtdump P_((SYMBOL *s,SYMBOL *sp));
static void pdecllist P_((void));
static TYPE *mkproto P_((SYMBOL *s));
static TYPE *mkprox P_((SYMBOL *s));
static NODE *datadef P_((SYMBOL *base,SYMBOL *s,SYMBOL *syment));
static void decllist P_((SYMBOL *base,SYMBOL *defbase,SYMBOL *d,SYMBOL *s,NODE **root,NODE **tail));
static int isdecl P_((void));
static TYPE *qarray P_((TYPE *t,int qflags,int *oflagp));
static int pbase P_((SYMBOL *symp));
static TYPE *tagspec P_((int typ));
static void sdeclenum P_((SYMBOL *tag));
static int sdeclstruct P_((SYMBOL *tag,int typ));
static SYMBOL *sdeclaration P_((SYMBOL *tag,SYMBOL *prevsmem,int *offset,int *boffset));
static int fldsize P_((SYMBOL *d, int bsiz,int *offset,int *boffset));
static SYMBOL *declarator P_((SYMBOL *d));
static TYPE *fundecl P_((SYMBOL *d,int dflag));
static void pidentlist P_((struct protostate *ps));
static TYPE *paramlist P_((struct protostate *ps));
static TYPE *addpp P_((TYPE *pp,TYPE *t));
static TYPE *pushsztype P_((int typ,int flags,int siz,TYPE *ptr));
static NODE *dodecl P_((int baseclass,SYMBOL *d,SYMBOL *s));
static NODE *pizer P_((SYMBOL *s));
static NODE *piztype P_((TYPE *t,int lev));
static NODE *chkarith P_((TYPE *t,NODE *e,int lev,int noptyp));
static NODE *pizstruct P_((TYPE *t,int lev,int isunion));
static NODE *pizarray P_((TYPE *t,int lev));
static NODE *pexizer P_((int lev));
static NODE *pizlist P_((void));
static void pizflush P_((int typ));
static int nisconst P_((NODE *e));
#if 0
static NODE *chkptrcon P_((NODE *e));
#endif
static int isauto P_((SYMBOL *s));
static void errtwotyp P_((SYMBOL *d,SYMBOL *s));
static void errdupsym P_((SYMBOL *s));
	...
	*s;	/* Symtab entry for parsed identifier */
{
	...
	*/
	...
	if (def) prtdump(s, d->Spmnext); /* If def, check for proto output */
	return s;			/* All done! */
    }
    ...
    ** class.
    */
    oproto = s->Stype->Tproto != 0; /* Remember if any previous proto */
    ...
    if (def && !oproto) prtdump(s, d->Spmnext);
				/* If def and no old proto, check for output */
    return s;
}

    ...
    */
	...
	for (n = 1; t1 && t2; t1 = t1->Tproto, t2 = t2->Tproto, ++n)
	    if (t1->Tspec != t2->Tspec
	      || (t1->Tspec == TS_PARAM && !tcomposite(t1->Tsubt, t2->Tsubt))) {
		if (flag) warn("Type of param #%d (%T) conflicts with prior def (%T)",
			       n, ParamSubt(t1), ParamSubt(t2));
		else error("Type of param #%d (%T) conflicts with prior def (%T)",
			   n, ParamSubt(t1), ParamSubt(t2));
		found++;
	    }
    if (t1 != t2) {
...
TYPE *nt, *ot;
{
    ...
    for (; s && nt && ot; s = s->Spmnext, nt = nt->Tproto, ot = ot->Tproto)
	if (!nt || !ot || nt->Tspec != ot->Tspec || nt->Tsubt != ot->Tsubt) {
	    warn("Type of parameter \"%s\" (%T) conflicts with prior prototype (%T)",
		 s->Sname, ParamSubt(nt), ParamSubt(ot));
	    found++;
	}
    if (s || nt || ot)
...
TYPE *t;
{
    ...
    } else for (n = 1; t; t = t->Tproto, n++) {
	if (t->Tspec != TS_PARAM) {
	    warn("Prototype param #%d (...) not compatible with default type",
		 n);
	}
	else if (!tcomposite(t->Tsubt,convfparam(t->Tsubt))) {
	    warn("Prototype param #%d (%T) not compatible with default type (%T)",
		 n, t->Tsubt, convfparam(t->Tsubt));
	}
    }
}

/* Prototype output routines */
...
{
  if (i < NTOKDEFS) {
    if (tokstr[i]) fputs(tokstr[i], fprt);
    else fprintf(fprt, "?Token=%s?", nopname[i]);
  }
  else fprintf(fprt, "?Token=%d.?", i);
}

/* PRTINDENT - Output current indentation.
*/
static void
prtindent(in)
int in;
{
    ...
    if (in > 0) {
      prttok(T_EOL);
      while (tabs--) putc('\t', fprt);
      while (spaces--) putc(' ', fprt);
    }
    else putc(' ', fprt);
}

/* PRTSIMPLE - Output simple type description.
*/
static void
prtsimple(t, needwsp)
TYPE *t;
int *needwsp;
{
    ...
    case TS_CHAR:
    case TS_UCHAR:
	if (tbitsize(t) == tgcsize) prttok(T_CHAR);
	else fprintf(fprt, "_KCCtype_char%d", tbitsize(t));
	break;
    case TS_SHORT:
    case TS_USHORT:
	prttok(T_SHORT);
    ...
    case TS_UNION:
    case TS_ENUM:
	fputs(tsnames[t->Tspec], fprt);
	prttok(T_WSP);
	fputs(t->Tsmtag->Sname, fprt);
	break;
    case TS_PARVOID:
	prttok(T_VOID);
	break;
	...
	break;
    default:
	if (t->Tspec < TS_MAX) fputs(tsnames[t->Tspec], fprt);
	else fprintf(fprt, "?Type=%d.?", t->Tspec);
	break;
    }
    (*needwsp)++;
}

/* PRTLPAF - Output left side of pointers, arrays, and functions.
*/
static void
prtlpaf(t, needwsp)
TYPE *t;
int *needwsp;
{
    while (t && ((t->Tspec == TS_FUNCT)
		 || (t->Tspec == TS_PARAM)
		 || (t->Tspec == TS_ARRAY)))
	t = t->Tsubt;

    if (t->Tspec == TS_PTR) {
	prtlpaf(t->Tsubt, needwsp);
	if (*needwsp) prttok(T_WSP);
	if ((t->Tsubt->Tspec == TS_FUNCT) || (t->Tsubt->Tspec == TS_ARRAY))
	    prttok(T_LPAREN);
	putc('*', fprt);
	*needwsp = 0;
	if (tisconst(t)) {
...
/* PRTRPAF - Output right side of pointers, arrays, and functions.
*/
static void
prtrpaf(sp, t, tp, in)
SYMBOL *sp;
...
int in;
{
    switch (t->Tspec) {
    case TS_PARAM:
	prtrpaf((SYMBOL *)NULL, t->Tsubt, (TYPE *)NULL, in);
	break;
    case TS_PARVOID:
    case TS_PARINF:
	break;
	...
	while (tp) {
	    prtindent(in);
	    prtspec(sp, NULL, tp, NULL, in > 0 ? in+2 : in);
	    if (tp = tp->Tproto) prttok(T_COMMA);
	    if (sp) sp = sp->Spmnext;
	}
	prttok(T_RPAREN);
	prtrpaf((SYMBOL *)NULL, t->Tsubt, (TYPE *)NULL, in);
    ...
    case TS_ARRAY:
	prttok(T_LBRACK);
	if (t->Tsize) fprintf(fprt, "%d", t->Tsize);
	prttok(T_RBRACK);
	prtrpaf((SYMBOL *)NULL, t->Tsubt, (TYPE *)NULL, in);
	break;
    case TS_PTR:
...
/* PRTSPEC - Output type specification.
*/
static void
prtspec(s, sp, t, tp, in)
SYMBOL *s;
SYMBOL *sp;
TYPE *t;
TYPE *tp;
...
/* PRTDUMP - Output function declaration suitable for prototyping.
*/
static void
prtdump(s, sp)
SYMBOL *s;
SYMBOL *sp;
{
    if (!(ptyp & ((s->Sclass == SC_INTDEF)	/* static definition? */
		   ? PTYP_STATIC		/* yes, was static selected? */
		   : PTYP_EXTERN)))		/* no, was extern selected? */
	return;					/* don't output this one */
    fprintf(fprt, "\n/* Function: %s */\n", s->Sname);
    prttok((s->Sclass == SC_INTDEF) ? T_STATIC : T_EXTERN);
    prttok(T_WSP);
    prtspec(s, sp, s->Stype, s->Shproto, 2);
    prttok(T_SCOLON);
    prttok(T_EOL);
}

/* DBGSPEC -- Debugging routine to output type specification from DDT
*/
TYPE *_dbgtype;
SYMBOL *_dbgsym;
void
_dbgspec()
{
  ...
  fprt = stdout;
  prtspec(_dbgsym, 0, _dbgtype, 0, 0);
  fprintf(fprt, "\n");
  fprt = savfprt;
}

/* PRTCHK - Check declaration for function type with no prototype.
 */
void
prtchk(t, s, pn)
TYPE *t;
...
int pn;
{
  ...
  if (clevel < CLEV_STDC) return; /* No check if less than STDC */
  ...
  do switch (t->Tspec)
	...
	if (s) {
	  if (pn) advise(
"Missing function prototype in declaration of parameter %S (#%d)", s, pn);
	  else advise("Missing function prototype in declaration of %S", s);
	} else advise("Missing function prototype in cast");
      }
    case TS_PARAM:
    case TS_ARRAY:
    case TS_PTR:
      t = t->Tsubt;		/* Follow subtype pointer */
      break;
    default:
      return;			/* Done if doesn't have subtype */
    } while (t);
}

...
pdecllist()
{
	...
	while (1) {
	    ...
	    prtchk(s1->Stype, s1, 0);	/* Check for missing prototype */
...
/* QARRAY() - returns the original type with 'qflags' applied to a
**	non-array sub-type.
...
static TYPE *
qarray(t, qflags, oflagp)
TYPE *t;
int qflags;
int *oflagp;
{
  if (t->Tspec == TS_ARRAY)
    return findctype(t->Tspec, t->Tflag,
		     t->Tsize, qarray(t->Tsubt, qflags, oflagp));
  return findctype(t->Tspec, qflags | (*oflagp = t->Tflag),
		   t->Tsize, t->Tsubt);
}
...
{
    SYMBOL *lastmem = tag;
    int maxsize = 0, offset = 0, boffset = 0;
    int savpok = paramok;

    paramok = 0;			/* Not OK to collect param syms */
    while (!eof && token != T_RBRACE) {
	lastmem = sdeclaration(tag, lastmem, &offset, &boffset);
	...
	if (offset > maxsize)		/* Remember size of largest element */
	    maxsize = offset;
	offset = boffset = 0;		/* Start over with zero offset */
    }
...
*/
static SYMBOL *
sdeclaration(tag, prevsmem, offset, boffset)
SYMBOL *tag, *prevsmem;
int *offset, *boffset;
{
    SYMBOL base, tempsym, *u, *s;
    int offcode, bsiz, ts, nelem, bpw, odd;
	...
	if (token == T_COLON) {
	    ...
	    */
	    ...
	    fldsize(NULL, pconst(), offset, boffset); /* parse & handle size */
	} else {
	    ...
	    */
		...
		if (tag->Stype->Tspec == TS_UNION
		  && clevel < CLEV_ANSI)
		    error("Bit-field %S not allowed in union", u);
		switch (tempsym.Stype->Tspec) {
		    default:
			error("Bit-field %S has invalid type", u);
		...
		if (bsiz > tbitsize(tempsym.Stype)) {
		    warn("Width of bit-field %S (%d) exceeds its type (%d)",
			 u, bsiz, tbitsize(tempsym.Stype));
		    bsiz = tbitsize(tempsym.Stype);
		}
		offcode = fldsize(u, bsiz, offset, boffset);	/* Handle it */
		...
		if (tempsym.Stype->Tspec == TS_VOID) {
		    error("Struct/union member %S cannot be void", u);
		    tempsym.Stype = deftype;
		}

		bsiz = tisscalar(tempsym.Stype) ?	/* Get object size */
			tbitsize(tempsym.Stype)		/* in bits if can */
			: (tisbytearray(tempsym.Stype)	/* Byte array? */
			   ? elembsize(tempsym.Stype) 	/* Yes, elem. size */
		...
		** in byte mode.  If the size is >= to a word, then we
		** will leave byte mode if needed.
		*/
		if (bsiz < TGSIZ_WORD) {
		    if (*boffset % bsiz)	/* Align to byte bndry */
			*boffset += bsiz - (*boffset % bsiz);
		    offcode = fldsize(u, bsiz, offset, boffset);
		    nelem = sizearray(tempsym.Stype) - 1;
		    if (nelem > 0) {
			odd = (TGSIZ_WORD - *boffset) / bsiz;
			if (nelem <= odd) *boffset += (nelem * bsiz);
			else {
			    nelem -= odd; /* Fill remainder of current word */
			    bpw = TGSIZ_WORD / bsiz;
					/* Compute bytes per word */
			    *offset += (nelem / bpw) + 1;
					/* Use partial and full words */
			    *boffset = bsiz * (nelem % bpw);
	    ...
	    */
		...
		prtchk(u->Stype, u, 0);	/* Check for missing prototype */
	    }
	}
	if (token != T_COMMA) break;
	nextoken();
...
*/
static int
fldsize(d, bsiz, offset, boffset)
SYMBOL *d;
int bsiz, *offset, *boffset;
{
    if (bsiz > TGSIZ_WORD || bsiz < 0)	/* range check */
	error("Bit-field %S longer than word (%d bits)", d, TGSIZ_WORD);
    ...
    */
	...
	case T_LPAREN:		/* Function definition or reference */
	    ...
	    */
	    ...
	    /* 4-Feb-93 RPH added test on !pp below to free parameter symbol
		list for pointers to or arrays of functions */
	    pp = addpp(pp, fundecl(d,
		(paramok && !pp && !inproto && d->Sclass != SC_TYPEDEF)
		? 0 : DF_INPROTO));
...
struct protostate *ps;
{
    ...
    */
    ...
    prtchk(t, s, ps->nparams);		/* Check for missing prototype */
	...
	*s;	/* Symbol table entry for the identifier */
{
    ...
    */
	...
	case TS_FUNCT:		/* Function reference or typedef */
		...
		prtchk(s->Stype, s, 0);	/* Check for missing prototype */
...
int lev;			/* Level being parsed.  0 - outermost */
{
	...
	case TS_PTR:
	    ...
	    if (!cmpgetype(t, e->Ntype)) {	/* Types must match */
		error("Pointer (%T) initializer has wrong type (%T)",
		      t, e->Ntype);
		break;
	    }
...
/* RPH changed 3-Jan-94 to allow pointers cast to integers
 *	    if ((!izautof || lev) && e->Nop != noptyp)
 *		error("%s constant required as initializer",
 *			(noptyp == N_ICONST ? "Integer" : "Floating-point"));
 * any illegal casts are caught at code generation time.
 */

    if ((!izautof || lev) && !nisconst(e))
	error("%s initializer not constant",
		(noptyp == N_ICONST ? "Integer" : "Floating-point"));
    return e;
}

...
int isunion;			/* TRUE if parsing union izer, else struct */
{
	...
	if (e && !cmpgetype(t, e->Ntype)) {
	    error("Struct/union (%T) initializer has wrong type (%T)",
		  t, e->Ntype);
	    pizlist();
	}
	return e;
...
{
    error("Symbol %S (type %T) previously declared with different type (%T)",
	  s, ParamSubt(d->Stype), ParamSubt(s->Stype));
}
