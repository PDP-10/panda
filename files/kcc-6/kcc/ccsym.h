...
SYMBOL {
  struct {
	...
	struct {		/* Handy values for macro defs (SC_[P]MACRO) */
...
/* Sclass == SC_[P]MACRO (preprocessor macro definition)
...
#define SF_SIDEFF	020	/* Side-effect (tag or enum) during parsing.
				...
				*/
#define SF_MACEXP	0100	/* SC_[P]MACRO only: macro being expanded.
...
#define SF_MACSHADOW	0200	/* Any class, indicates this symbol is
				...
				** same identifier).
				*/
#define SF_MACRO	040	/* SC_[P]MACRO only: overloading class flag */
...
/* Test for parameter and get subtype */
#define ParamSubt(t) ((t)->Tspec == TS_PARAM ? (t)->Tsubt : (t))
