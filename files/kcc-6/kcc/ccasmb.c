...
static char *gtmpfile P_((char *name));
#if SYS_T20
static int gprarg P_((int *loc,int len));
#endif
static int *stmpfile P_((char *name,char *str,char *nextprog));
static int hackfork P_((char *pgmname,int *argblk,int blklen,int stoffset,int chainf));
static int _mov7 P_((char *path));
static int file7 P_((char *path));
static int cvt10 P_((char *path));
static char *fnconv P_((char *dst,char *src));
static char *fnlibsrch P_((char *dst,char *libpath,char *src));
static int ldsymfile P_((char *fnam));
static int crsfunv P_((char *tabp, long flen));
...
char *m, *f1, *f2;
{
	...
	res = hackfork((tgasm==TGASM_FAIL  ? "FAIL" : "MACRO"),
		    stmpfile((tgasm==TGASM_FAIL  ? "FAI" : "MAC"), str, NULL),
		    PRGBLEN,		/* Use PRARG */
    ...
    */
    ...
    if (res > 0) jerr("Could not run assembler - %s", strerror(res));
    else if (res < 0) {
	res &= ~0400000000000;	/* Clear sign bit */
	if (WIFEXITED(res))
	  jerr("Assembler exited with error - %d", WEXITSTATUS(res));
	else if (WIFSIGNALED(res)) {
	  extern char *sys_siglist[];

	  if (WTERMSIG(res) < NSIG)
	    jerr("Assembler terminated due to signal - %s",
		 sys_siglist[WTERMSIG(res)]);
	  else
	    jerr("Assembler terminated du to signal - unknown signal %d",
		 WSTOPSIG(res));
	}
    }
...

/* addargs - Add arguments to the FRONT of the argv array.
**      Passed a string of arguments which are broken at whitespace.
*/
int addargs(argcp, argvp, newargs)
int *argcp;
char ***argvp, *newargs;
{
  char **newargv = NULL, *arglst, *argptr, c, qc;
  int argcnt = 0, i, j;

  argptr = arglst = malloc(strlen(newargs) + 1);
  if (!arglst) return 0;        /* Fail if no memory */

  do {
    while ((c = *newargs++) && isspace(c));
    if (!c) break;
    argcnt++;
    do {
      if (isspace(c)) break;
  ...
  newargv = (char **)malloc(sizeof(char *) * (*argcp + argcnt + 1));
  if (!newargv) return 0;       /* Fail if no memory */
  j = 0;
  if (*argcp > 0) newargv[j++] = (*argvp)[0];
  argptr = arglst;
  for (i = 0; i < argcnt; i++) {
    newargv[j++] = argptr;
    argptr = strchr(argptr, '\0') + 1;
  }
  for (i = 1; i < *argcp; i++)
    newargv[j++] = (*argvp)[i];
  newargv[j] = NULL;
...
char *name, *str, *nextprog;
{
    ...
    if (fputs(str, tf) == EOF)		/* Write out the string */
      errio("writing TMP", tmpfile);
    if (fclose(tf) == EOF)
      errio("writing TMP", tmpfile);
    if (cp) free(cp);
    return NULL;
...

/* Compute the default psect length by comparing it to all other psect
 * beginning addresses that are in the same section. Note that we
 * compare against ourselves, but we compare less than or equal, and
 * are not considered for the end.
 */
static int
psectlen(psbeg)
{
  int psend, section = psbeg & LH;
  
  psend = section | PSMEM_LIM;	/* This is the universal in-section maximum */

  /* Check against beginning of code psect */
  if (((ldpscode.ps_beg & LH) == section)
      && (ldpscode.ps_beg < psend)
      && (ldpscode.ps_beg > psbeg))
    psend = ldpscode.ps_beg;

  /* Check against beginning of data psect */
  if (((ldpsdata.ps_beg & LH) == section)
      && (ldpsdata.ps_beg < psend)
      && (ldpsdata.ps_beg > psbeg))
    psend = ldpsdata.ps_beg;

  /* Check against beginning of symbol psect */
  if (((ldpssyms.ps_beg & LH) == section)
      && (ldpssyms.ps_beg < psend)
      && (ldpssyms.ps_beg > psbeg))
    psend = ldpssyms.ps_beg;
...
char *nextprog;			/* Next program to chain thru, if any */
{
    ...
    t = estrcpy(fnconv(t, ofilename), "/SAVE=");
#endif
	...
	/* Default psect starts */
	if (ldpsdata.ps_beg < 0)
	  ldpsdata.ps_beg = ldextf ? PSDATA_BEG | 01000000 : PSDATA_BEG;
	if (ldpscode.ps_beg < 0)
	  ldpscode.ps_beg = ldextf ? PSCODE_BEG | 01000000 : PSCODE_BEG;

	/* Default psect lengths */
	if (ldpsdata.ps_len < 0) ldpsdata.ps_len = psectlen(ldpsdata.ps_beg);
	if (ldpscode.ps_len < 0) ldpscode.ps_len = psectlen(ldpscode.ps_beg);
	  
	sprintf(t,"/SET:DATA:%o/LIMIT:DATA:%o/SET:CODE:%o/LIMIT:CODE:%o\n",
		ldpsdata.ps_beg,
		ldpsdata.ps_beg + ldpsdata.ps_len,
		ldpscode.ps_beg,
		ldpscode.ps_beg + ldpscode.ps_len);
	t += strlen(t);
	t = estrcpy(t, "/REDIRECT:DATA:CODE\n");
	if (ldpssyms.ps_beg < 0)
	  t = estrcpy(t, "/SYMSEG:PSECT:DATA\n");
	else {
	  if (ldpssyms.ps_len < 0) ldpssyms.ps_len = psectlen(ldpssyms.ps_beg);
	  sprintf(t,"/SET:.SYM.:%o/LIMIT:.SYM.:%o/SYMSEG:PSECT:.SYM.\n",
		  ldpssyms.ps_beg, ldpssyms.ps_beg+ldpssyms.ps_len);
	  t += strlen(t);
	}
    }
    if (ldronly)
	t = estrcpy(t, ldpsectf ? "/RONLY:CODE\n" : "/RONLY:.HIGH.\n");
    if (ldextf) {		/* Hacking extended addressing? */
	t = fnlibpath(t, "ckx"); /* -i Preload special x-addr module */
	*t++ = ',';
    }
	...
	if (*s == '-') switch (*++s) {	/* Is this a still-alive switch? */
		...
		t = fnlibpath(t, s+1);	/* Convert to linker format */
		t = estrcpy(t, "/SEARCH");
		break;
		...
		continue;		/* Put nothing in buffer */
	    }
	    else {
		/* Library? */
	        if (fnxislib(ext)) {
		    t = fnconv(t, s); /* Yes, convert to linker format */
		    t = estrcpy(t, "/SEARCH");
				/* Provide /SEARCH for libraries */
		}
		/* Object? */
	        else if (fnxisrel(ext)) t = fnconv(t, s);
				/* Yes, convert to linker format */
		/* Not library or object, */
		else t = estrcpy(s = t, module);
				/* replace with just module */
	    }
        }
...
char *nextprog;			/* Program to chain after LINK, if any */
{
    ...
    */
    ...
    res = hackfork(pname, loc, loc ? PRGBLEN : 0, 1, 1);
    fatal("Could not chain to %s - %s", pname, strerror(res));
}

...
int stoffset, chainf;	/* Starting offset, and chain flag */
{
    ...
    fx.fx_blklen = blklen;
    fx.fx_waitres = 0;
    if (forkexec(&fx) < 0) return errno;
				/* If forkexec fails, return minus error */
    if (fx.fx_waitres)		/* If status wasn't normal, */
      return (fx.fx_waitres | 0400000000000);
				/* Return result of subfork + 1B0 */
...
#if SYS_T20
/* For TOPS20, convert to TOPS10 format for linker */
...
static int jfnblk[] = {
  monsym("GJ%OFG") | monsym("GJ%XTN"), /* .GJGEN */
...
/* Convert TOPS20 filespec to TOPS10 format, result unchanged on error */
static int cvt10(char *path)
{
  int acs[5], jfn, ppn, ret = -1;
  ...
  if (jsys(GTJFN, acs)) {
    if (!*(char *)(acs[2])) {
      ...
      acs[1] = (int)(tbuf - 1);
      acs[2] = jfn;
      acs[3] = 0110000000001;	/* Device & directory */
      if (jsys(JFNS, acs)) {
	if (tbuf[strlen(tbuf)-1] != '>') ret = 0; /* No dir, no change */
	else {
	  acs[1] = file7(tbuf);
	  if (jsys(STPPN, acs)) {
	    ppn = acs[2];
...
/* FNCONV - convert filename to something palatable to the linker.
**	Uses abspath() to cananonicalize filename.
*/
static char *fnconv(dst, src)
char *dst;
char *src;
{
  filename result;

  if (!abspath(src, result))		/* Convert to NATIVE format */
    src = result;			/* If successful, use result */
#if SYS_T20
  cvt10(result);			/* Convert to TOPS10 format */
#endif
  return dst == src ? &src[strlen(src)] : estrcpy(dst, src);
}

static char * fnlibsrch(dst, libpath, src)
char *dst, *libpath, *src;
{
  char *tf;

  fstrcpy(dst, libpath, src);	/* Create library filename */
  if (access(dst, R_OK))	/* Can we read it? */
    return NULL;		/* No, return failure */

  tf = fnconv(dst, dst);	/* Convert filename as needed */

  return tf;			/* Return pointer to end */
}

/* FNLIB - construct/locate library file specification (-lxxx)
** and convert result using fnconv().
*/
char *fnlibpath(dst, src)
char *dst, *src;
{
  char *tf;
  int i;

  /* First search libraries specified with -L */
  for (i = 0; i < nhlibpaths; i++) { /* Loop thru user list */
    tf = fnlibsrch(dst, hlibpaths[i], src); /* Search for it */
    if (tf) return tf;		/* Return pointer to end */
  }

  /* Now search default libraries */
  for (i = 0; i < nihlibpaths; i++) { /* Loop thru system list */
    tf = fnlibsrch(dst, ihlibpaths[i], src); /* Search for it */
    if (tf) return tf;		/* Return pointer to end */
  }

  /* Not found, assume first entry in system list and pray */
  fstrcpy(dst, ihlibpaths[0], src); /* Construct default name */
  tf = fnconv(dst, dst);	/* Convert */
  jerr("Can't find library -l%s trying \"%s\"", src, dst);
  return tf;
}

...
*/
char *
fnparse(source, direc, name, ext, suf)
char *source, *direc, *name, *ext, *suf;
{
  char dir[FNAMESIZE];
  char atr[FNAMESIZE];
  int i;

  if (decomp(source, direc, dir, name, ext, suf, suf ? atr : NULL))
    return "Parse failed";

  if (direc && *dir) strcat(direc, dir); /* Combine dev and dir */
  if (suf && *atr) strcat(suf, atr); /* Combine gen and atr as suffix */

  if (name && !*name) {		/* If no name, must extract from dir */
    for (i = 0; dir[i]; i++) {
      if (dir[i] == '\026') {
	if (dir[++i]) continue;
	break;
      }
      if (isalnum(dir[i])) continue;
      if (dir[i] == '-') continue;
      if (dir[i] == '_') continue;
      break;
    }
    strncpy(name, dir, i);	/* Extract module from first field */
  }

  return NULL;		/* And indicate success with no msg */
}
...
{
#if SYS_ITS
    return !strcasecmp(cp, " " SWI_LIBTYPE);
#else
    return !strcasecmp(cp, "." SWI_LIBTYPE);
#endif
}
...
{
#if SYS_ITS
    return !strcasecmp(cp, " " SWI_OBJTYPE);
#else
    return !strcasecmp(cp, "." SWI_OBJTYPE);
#endif
}
