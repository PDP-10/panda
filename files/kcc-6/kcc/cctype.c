...
static int cast_op P_((TYPE *to,TYPE *frm));
static void convboth P_((NODE *n,TYPE *newtyp));
static NODE *convxboth P_((NODE *n,TYPE *newtyp));
static int convisnull P_((NODE *n));
static int nisnull P_((NODE **an));
static NODE *ndefcast P_((int op,TYPE *t,NODE *n));
...
/* Now done in cmpgetype */
...
#endif
	if (cmpgetype(lt->Tsubt, rt->Tsubt)
    ...
    */
	...
	warn("Implicit conversion - %T => %T", rt, lt);
    return cn;
}

