...
static void pass_where P_((void));
static void dotad P_((void));
static void pass_ident P_((FILE *fp));
static void pass_num P_((void));
static int pass_str P_((int cnt));
static void pass_hwsp P_((void));
static void pass_line P_((int suppress));
static void pass_comm P_((void));
static int nextch P_((void));
static int tchesc P_((void));
static void sinbeg P_((char *cp));
static void sinend P_((void));
static void pushch P_((int c));
static int pushstr P_((char *cp));
static void bstrpush P_((char *cp));
static void bstrpop P_((void));
static int nextrawpp P_((void));
static int scanhwsp P_((void));
static void scancomm P_((void));
static int ppnconst P_((int ch1));
static void ppsconst P_((int ch1));
static int ppunknwn P_((int eat));
static PPTOK *ppterror P_((void));
static int findident P_((void));
static SYMBOL *findmacsym P_((char *id,int private));
static int nextmacpp P_((void));
static void pushmp P_((void));
static void mtlpush P_((tlist_t tl));
static void mtlpop P_((void));
static char *tltomac P_((tlist_t tl,char *cp));
static tlist_t tlfrmac P_((char *cp));
static char *tltostr P_((tlist_t tl,char *cp,int n));
static tlist_t tlfrstr P_((char *cp));
static tlist_t tlimake P_((int val,int baseflg));
static PPTOK tokize P_((void));
static void tlrawadd P_((tlist_t *atl));
static tlist_t tlcopy P_((tlist_t il));
static tlist_t tlstrize P_((tlist_t tl));
static int pptfput P_((PPTOK *p,FILE *fp));
static char *sltostr P_((PPTOK *p,char *cp,int max));
static void ppcqstr P_((char *cp));
static void ppctstr P_((PPTOK *p));
static int tkerr P_((char *rtn,int i));
static int ppcerror P_((void));
static int mexptop P_((SYMBOL *sym,int hs));
static int mpeekpar P_((void));
static tlist_t mexpsym P_((SYMBOL *sym,int hs));
static int margs P_((tlist_t *tabptr,int maxsto,int nexpected));
static tlist_t mexplim P_((tlist_t il,int hs));
static tlist_t mexpand P_((tlist_t il,int hs));
static tlist_t msubst P_((struct macframe *mf,int hs));
static tlist_t mpaste P_((tlist_t tl,int hs,int ncats));
static int hspush P_((SYMBOL *sym,int hs));
static int tkhide P_((PPTOK *p,int hs));
static int mishid P_((SYMBOL *sym,int hs));
#if DEBUG_PP
static char *plevindent P_((void));
static void pmacframe P_((char *str,struct macframe *mf));
static void pmactl P_((char *s,tlist_t tl,int hs));
static void tlfput P_((tlist_t tl,FILE *fp,int vrbflg));
static char *tkids P_((PPTOK *p));
static void tkfput P_((PPTOK *p,FILE *fp,int vrbflg));
#endif
static void directive P_((void));
static int d_define P_((int private));
static int mdefinp P_((struct macframe *mf));
static SYMBOL *mdefstr P_((char *name,int mactyp,char *body));
static SYMBOL *mdefsym P_((char *name,struct macframe *mf,int private));
static int d_undef P_((int private));
static void freemacsym P_((SYMBOL *sym));
static int d_asm P_((void));
static tlist_t asmrefill P_((void));
static int d_endasm P_((void));
static int d_ifdef P_((int cond,int private));
static int d_if P_((void));
static int d_elif P_((void));
static int d_else P_((void));
static int d_endif P_((void));
static void flushcond P_((void));
static int iftest P_((void));
static void iffwarn P_((char *typ));
static void ifpush P_((int inlev));
static void ifpopchk P_((int inlev));
static int d_include P_((void));
static int cinctry P_((int n,char **ptab,char *f2,char *f,FILE **fp,char *sep));
static void filepush P_((FILE *fp,char *fname,int lin,int pag,int flin));
static int d_line P_((void));
static int d_error P_((void));
static int d_pragma P_((void));
static int getfile P_((char *f,int n));
static int tskipwsp P_((void));
static int tskiplwsp P_((void));
static int flushtoeol P_((void));
static int cskiplwsp P_((void));
static void checkeol P_((void));
#if 0
static int getidrest P_((char *str));
#endif
static tlist_t getlinetl P_((void));
static tlist_t tlwspdel P_((tlist_t tl,int allf));
static tlist_t tlmake P_((int typ,union pptokval val));
...
#define MAXBKSTRS (MAXINCLNEST+MAXINCFIL+2)	/* Rough guess */
...
#endif

/* PASS_WHERE - used during PASSTHRU() to output file context.
**	Expects filename in 'inpfname' current line in 'fline'.
*/
static FILE *passfp;

#define INCHEAD "#include \""
#define INCTAIL "\"\n"
...
ppinit()
{
    ...
    for (i=nincfiles-1; i >= 0; i--) { /* Ensure processed in order given */
      incs = malloc(strlen(incfiles[i])+sizeof(INCHEAD INCTAIL));
      if (!incs) error("Out of memory for -I=file");
      else {
	sprintf(incs, INCHEAD "%s" INCTAIL, incfiles[i]);
	bstrpush(incs);		/* Push "#include file\n" */
      }
    }
	...
	*defcsname = SPC_MACDEF;	/* Hide sym by smashing 1st char */
    ...
    switch (tgsys) {
    case TGSYS_TOPS20: tgsysstr = "TOPS20"; break;
    case TGSYS_TOPS10: tgsysstr = "TOPS10"; break;
    case TGSYS_WAITS: tgsysstr = "WAITS"; break;
    case TGSYS_TENEX: tgsysstr = "TENEX"; break;
    case TGSYS_ITS: tgsysstr = "ITS"; break;
    default: return;		/* Unknown target system, no definition */
    }
    mdefstr(tgsysstr, MACF_ATOM, "1"); /* Define target system type */
}
...
char **utab, **dtab;	/* Address of char-pointer array */
{
    ...
    for ( ; --unum >= 0; utab++) {
	if (sym = findmacsym(*utab, 0))	/* Look up the macro symbol */
	...
	*cp = '\0';			/* Tie name off in case of '=' */
	if (sym = findmacsym(*dtab, 0)) { /* See if already defined */
...
FILE *fp;
{
    ...
    pass_where();		/* Mark beginning */

    pass_line(1);		/* Suppress first newline */
...
FILE *fp;
{
    ...
    if (!(sym = findmacsym(cp, -1)) /* If not a macro, */
...
pass_line(suppress)
{
	...
	for (;;) {			/* Flush whitespace and comments */
	    switch (nextch()) {
	    /* RPH - 14-Oct-93 - No longer treats comments like whitespace. */
	    /*This is consistent with other C preprocessors */
	    case ' ': case '\t':
...
pass_comm()
{
    ...
    if (clevel >= CLEV_ANSI) ch = ' ';	/* Pretend current char now a space */
    else nextch();			/* Else get following char */
}

#if 0
...
nextch()
{
    ...
    *erptr++ = ch;		/* Add to saved input line. */
	...
	if (inlevel > 0) {		/* Drop level.  If more, pop input. */
	    ...
	    if (prepf) pass_where();	/* Output file context */
#if DEBUG_PP
	    if (debpp) {
...
#define retchk2(ch1,ch2,no,yes1,yes2) \
	...
	return ppunknwn(0);	/* Else return unknown token */

    case '\\':			/* Backslash is unknown, but eats next char */
	return ppunknwn(1);	/* Eat following char and return unknown */
	
    default:
	if (!iscsymf(ch))	/* Can char be start of identifier? */
	    return ppunknwn(0);	/* Nope, return unknown token */
...
static int
ppunknwn(eat)
int eat;			/* Number of following chars to eat */
{
    rawval.cp = ppcbeg();	/* Get ptr into char pool */
    do {
      ppcput(ch);
      nextch();			/* Consume it, get next */
    } while (eat--);		/* Loop until specified chars eaten */
    rawpplen = ppcend();	/* Done with string */
...
findident()
{
    ...
    */
    ...
    while ((cursym->Sclass == SC_MACRO) || (cursym->Sclass == SC_PMACRO)) {
	...
	** Additional hair by RPH: Must loop to handle private macro
	** shadowed by normal macro.
	*/
	cursym = symfnext(cursym,0); /* Find next sym. not a macro */
	if (!cursym) {		/* If none, */
...
*/
static SYMBOL *
findmacsym(id, private)
char *id;
int private;			/* -1 find normal & private */
				/* 0  find normal only */
				/* 1  find private only */
{   SYMBOL *macsym;

    if (macsym = symfind(id, 0)) {	/* Scan for macro or identifier */
	if ((private > 0) && (macsym->Sclass == SC_MACRO))
					/* If seeking private & got normal */
	    macsym = symfnext(macsym,SF_MACRO);	/* try for private version */
	if (((macsym->Sclass == SC_MACRO) && (private <= 0)) ||
	    ((macsym->Sclass == SC_PMACRO) && (private != 0)))
					/* Found symbol, see if macro */
					/* of desired type */
...
tlist_t tl;
{
    union pptokval tmpval;

    tmpval.cp = ppcbeg();	/* Start deposit into token char pool */
    ppcput('"');
    for (; tlpcur(tl); tlpskip(tl))
    ...
    */
	...
	switch (mf.mf_nargs) {
		...
		*/
		...
		tmpval.cp = findmacsym(rawval.cp, -1) ? "1" : "0";
		tl = tlmake(T_ICONST, tmpval);	/* Expand into true or false */
		if (parens) {
		...
		if (i != 2
		  ...
		  || tlpcur(tl2)->pt_nxt) {
			...
			symv? "val" : "fnd");
		    tmpval.cp = "0";
		    return tlmake(T_ICONST, tmpval); /* Return 0 if err */
		} else {
...
PPTOK *p;
{
    ...
    if (p->pt_typ == T_IDENT) {
	if (!(sym = findmacsym(p->pt_val.cp, -1)))
...
directive()
{
	...
	*/
	...
	case 5:
	    ...
	    else if (!strcmp(rawval.cp,"ifdef")) res = d_ifdef(1, 0);
	    else if (!strcmp(rawval.cp,"undef"))
		res = flushing ? PPR_FLUSH : d_undef(0);
	    else if (!strcmp(rawval.cp,"error"))
		res = flushing ? PPR_FLUSH : d_error();
	    break;
	case 6:
		 if (!strcmp(rawval.cp,"ifndef")) res = d_ifdef(0, 0);
	    else if (!strcmp(rawval.cp,"define"))
		res = flushing ? PPR_FLUSH : d_define(0);
...

/* D_DEFINE(private) - Process #define directive
*	private		Use by #pragma to create "private" macros
*/
static int
d_define(private)
int private;
{
    ...
    *defcsname = 'd';			/* Allow "defined" to be found */
    sym = findmacsym(name, private);	/* Find normal or private only */
	...
	*/
	if (private) warn("Private macro redefined: \"%s\"", name);
	else warn("Macro redefined: \"%s\"", name);
	freemacsym(sym);		/* Flush old sym completely */
    }

    /* Now define the new macro! */
    mdefsym(name, &m, private);
    return PPR_CHECKEOL;
}
...
struct macframe *mf;
{
	...
	case T_SHARP:
	    if (clevel < CLEV_ANSI) goto ordinary; /* Only special if ANSI */
	    if (tskipwsp() != T_IDENT) {
...
char *body;			/* Text of macro definition */
{
    ...
    return mdefsym(name, &m, 0); /* Always define macro, and return! */
}
...
int private;
{
	...
	s = ((s->Sclass == SC_MACRO) && private)
	  ? shmacsym(symgcreat(name)) /* and move SC_MACRO to front */
	    : symgcreat(name);
    }
    s->Sclass = private ? SC_PMACRO : SC_MACRO;
				/* Say it's a macro of specified type */
...

/* D_UNDEF(private) - Process #undef directive
*	private		Use by #pragma to create "private" macros
*/
static int
d_undef(private)
int private;
{
    ...
    *defcsname = 'd';			/* Allow "defined" to be found */
    sym = findmacsym(rawval.cp, private); /* Find normal or private only */
    *defcsname = SPC_MACDEF;		/* Hide "defined" again */
    if (sym != NULL) {			/* If already defined, */
	if (sym->Smacnpar >= MACF_ATOM)	{ /* Check for specialness */
	    freemacsym(sym);		/* OK to flush it, do so. */
	    if (!private && (sym = symfidstr(rawval.cp)))
	      shmacsym(sym);		/* Make sure SC_PMACRO is at front */
	}
...
d_asm()
{
    ...
    tmpval.i = 0;
    curtl = tlmake(T_EOL, tmpval);	/* Set up curtl with dummy */
    return PPR_CHECKEOL;
}
...
asmrefill()
{
    ...
    tmpval.cp = "asm";
    tl = tlmake(T_IDENT, tmpval); /* Make start of returned tokenlist */
...
/* D_IFDEF(cond, private) - Process #ifdef and #ifndef directives
*/
static int
d_ifdef(cond, private)
int cond;		/* 1 == ifdef, 0 == ifndef */
int private;		/* 0 == normal, 1 == private */
{
    ...
    if (cond == ((findmacsym(rawval.cp, private) != NULL)))
...
flushcond()
{
    ...
    do {
	ppcsave(savppc);	/* Save token state */
	if (tskipwsp() == T_SHARP) /* Check start of line */
	    directive();	/* Handle directive! */
	else flushtoeol();
	ppcrest(savppc);	/* Restore token state */
...
#define fs NULL
	...
	*/
	...
	/* If not at top level, look in dir of current file */
        if ((inlevel > 0) && !fnparse(inpfname, f2, NULL, NULL, fs)) {
	    strcat(f2,f);	/* Tack on file we are looking for */
#if SYS_T10
	    strcat(f2,fs);	/* Add suffix for TOPS-10 */
#endif
	    if (fp = fopen(f2, "r")) /* Try to open it */
		++done;		/* Flag found */
	}

	/* If not found, look in directory of top level file */
	if (!done) {
    ...
    */
	...
	/* Unsuccessful open, try -I switches if any, in order given */
	if (nincpaths) done = cinctry(nincpaths, incpaths, f2, f, &fp, "/");
	...
	*/
	if ((nhfsypaths || nihfsypaths) && (strncmp(f, "sys/", 4) == 0)
	  && (nhfsypaths <= 0
		|| !cinctry(nhfsypaths, hfsypaths, f2, f+4, &fp, 0)))
	    cinctry(nihfsypaths, ihfsypaths, f2, f+4, &fp, 0);

	/* If not, or didn't succeed, just handle basic <> file with -H sws.
	** Note that default entries (if they exist) are tried last.
	*/
	if (!fp
	  && (nhfpaths <= 0 || !cinctry(nhfpaths, hfpaths, f2, f, &fp, 0)))
	    cinctry(nihfpaths, ihfpaths, f2, f, &fp, 0);
    }
...
**	of directory pathnames. 'sep' is explicit separation string
**	between path and file, and disables "+" construct.
**	Succeeds if file is opened OR if a NULL entry is encountered.
*/
static int
cinctry(n, ptab, f2, f, fp, sep)
register int n;			/* # paths in table */
char **ptab;			/* Addr of table */
char *f2, *f;			/* Places to deposit built and orig names */
FILE **fp;
char *sep;			/* Explicit separation string */
{
    for(; --n >= 0; ++ptab) {
	if (!*ptab || !**ptab)		/* If we hit enforced stop, */
	    return 1;			/* always done now. */
	if (sep) estrcpy(estrcpy(estrcpy(f2, *ptab), sep), f);
	else fstrcpy(f2, *ptab, f);	/* Build filename to try */
...
*	Currently used to implement "private" versions of the following:
*
*		#pragma private define
*		#pragma private undef
*		#pragma private ifdef
*		#pragma private ifndef
*
*	"private" macros live between "normal" macros and symbols.
*/
static int
d_pragma()
{
    ...
    if ((tskipwsp() == T_IDENT) && !strcmp(rawval.cp,"private")) {
	len = (tskipwsp() == T_IDENT ? strlen(rawval.cp) : 0);

	switch (len) {
	case 5:
		 if (!strcmp(rawval.cp,"ifdef")) return d_ifdef(1, 1);
	    else if (!strcmp(rawval.cp,"undef")) return d_undef(1);
	    break;

	case 6:
		 if (!strcmp(rawval.cp,"ifndef")) return d_ifdef(0, 1);
	    else if (!strcmp(rawval.cp,"define")) return d_define(1);
	    break;
...
flushtoeol()
{
	...
	case '/': if (nextch() == '*')
		    scancomm();
		  else break;	/* Already have next char */
	default: nextch();
    }
}
