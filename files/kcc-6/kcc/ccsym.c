...
static void inisymlist P_((SYMBOL **ahead,SYMBOL **atail));
static void makelsym P_((SYMBOL *s));
static void makegsym P_((SYMBOL *s));
static SYMBOL *mksym P_((char *id,SYMBOL **tailptr));
static SYMBOL *symmk P_((SYMBOL *s,int hval,SYMBOL **tailptr));
static SYMBOL *getsym P_((SYMBOL **tailptr));
static void retsym P_((SYMBOL *syment));
static int idcmp P_((char *s,char *t));
static int symcmp P_((SYMBOL *s1,SYMBOL *s2));
static int idcpy P_((SYMBOL *s,char *cp));
static void smapinit P_((void));
static int smapmatch P_((mpdsym ms));
static void labinit P_((void));
static void realfreelabel P_((SYMBOL *lab));
static void ambput P_((char *str));
static void shoffset P_((int off));
static void typeinit P_((void));
static TYPE *tcomproto P_((TYPE *t1,TYPE *t2));
static void aryerr P_((char *s));
...
SYMBOL **ahead, **atail;
{
	...
	for (head = (*ahead)->Snext; s = head;) {	/* For all but 1st */
	    if (((s->Sclass == SC_MACRO) || (s->Sclass == SC_PMACRO))
		&& s->Smacptr)
...
BUGMSG(("symfidstr \"%s\"\n", str))
	...
	while ((sym->Sclass == SC_MACRO) || (sym->Sclass == SC_PMACRO))
					/* If got macro, */
	    sym = symfnext(sym,0);	/* get non-macro instead if any */
    }
    return sym;
}
...
** not an identifier string. The flag argument allows the specified
** flags to be on and still generate a match (used to find private
** macros).
*/
SYMBOL *
symfnext(osym,flag)
...
BUGMSG(("symfnext \"%s\"\n", sym->Sname))
	...
	if (((sym->Sflags&((SF_XLOCAL|SF_OVCLS)&~flag)) == 0)
	 && symcmp(sym, osym)) {
	    sym->Srefs++;
	    break;
	}
...
**
**	Also called to move SC_MACRO in front of SC_PMACRO. Setting of
**	SF_MACSHADOW shoudl be harmless in this case.
**
...
#if 0
/* Removed 13-Jan-94 by R. Helliwell
 * It was preventing forward struct/union/enum reference from being
 * found again when declared.
 */
...
SYMBOL *s;
{
	...
	*cp = '%';		/* Always starts with this prefix */
    ...
    if (s->Sname[0] == SPC_IDQUOT) {	/* Treat `ident` specially */
      error("Quoted internal link name duplicated: %S", s);
      s->Smaplab = ms;			/* Use name anyway */
      return;
    }
...

/* AMBDUMP - Dump potentially ambiguous external symbols to .amb file.
*/
extern FILE *famb;
...
char *str;
{
    if (!famb) {		/* If output file not open yet, */
	if ((famb = fopen(ambfname, "w")) == NULL) {
	    errfopen("potentially ambiguous external symbol", ambfname);
	      famb = stdout;	/* Send to stdout instead */
	}
    }
    ambnum++;
    fputs(str, famb);		/* Output symbol name */
    fputc('\n', famb);		/* and newline */
}
...
SYMBOL *table;
{
    ...
    famb = NULL;		/* File not open yet */
    ambnum = 0;			/* None output yet */
    for (s = table; s != NULL; s = s->Snext)
	if (s->Sclass == SC_EXTDEF) {
	    str = s->Sname;
	    if (*str == SPC_IDQUOT) ++str; /* Skip 1st char if `ident` */
	    if (strlen(str) > 6) ambput(str);
	    else for (sp = str; *sp; sp++)
		if (isascii(*sp) && isupper(*sp)) {
		    ambput(str);
		    break;
		}
	}
    if (famb && (famb != stdout)) fclose(famb);
    if (ambnum) advise("Number of potentially ambiguous symbols = %d", ambnum);
}

...
char *name;
{
	...
	case SC_PMACRO:	str = "private macro"; break;
	...
	if (u != SC_MACRO && u != SC_PMACRO && u != SC_TAG && s->Stype) {
...
{    return findctype(TS_FUNCT, 0,	/* Always function, no qualifiers */
		(int)plist,		/* Param list - note type punning! */
		rtyp);			/* Return type */
}
...
/* CMPGETYPE - Compare two types for "greater than or equal"
** qualifiers. This is for assignments which must not relax the
** const/volatile constraints. The left (lt) must have the same or
** more constraints than the right (rt).
*/
int
cmpgetype(lt, rt)
TYPE *lt, *rt;
{
	...
	/* Win quick if equal now */
	if (lt == rt) return 1;

	/* Lose quick if not same basic type */
	if (lt->Tspec != rt->Tspec) return 0;

	/* Here we verify that any qualifiers used on right side of
	   assignment are also present on the left side. */
	...
	case TS_ARRAY:
	    ...
	    */
	    if (lt->Tsize && rt->Tsize && (lt->Tsize != rt->Tsize))
	        return 0;
	    /* Sizes OK, check subtype */
	case TS_FUNCT:
	case TS_PTR:
	    /* Just check subtype */
	    break;
	...
	case TS_STRUCT:
	case TS_UNION:
	    /* Structures and unions must have same tag list */
	    return (lt->Tsmtag == rt->Tsmtag);

	default:
	    /* Anything else is terminal type, and thus equal */
	    return 1;
	}
...
TYPE *t, *u;
{
	...
	/* Lose quick if not same basic type */
	if (t->Tspec != u->Tspec) return 0;

	/* Lose if flags differ (ignoring const/volatile) */
	if ((t->Tflag&(~TF_QUALS)) != (u->Tflag&(~TF_QUALS))) return 0;
