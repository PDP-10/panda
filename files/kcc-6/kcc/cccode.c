...
static void shoall P_((void));
static void shocurcod P_((void));
static void shohdr P_((char *loc,int op,int r));
static void shopcod P_((FILE *f,PCODE *p,int idx));
static void shoop P_((FILE *f,unsigned op));
static void shoupd P_((FILE *f));
static void shocum P_((void));
static void shocmp P_((FILE *f));
static void flsprev P_((void));
static void code40 P_((int op,int r,int s,int bsiz));
static void codr8 P_((int op,int r,int val));
static void codrmdx P_((int type,int op,int reg,SYMBOL *ptr,int offset,int index));
static void codrrx P_((PCODE *prev,PCODE *np));
static int rrpre1 P_((PCODE *p));
static int rrpre2 P_((PCODE *p,PCODE *np));
static void rrpre3 P_((PCODE *p,PCODE *np,int op));
static PCODE *chkref P_((PCODE *begp,PCODE *q,int r));
static void rrpopt P_((PCODE *p));
static void rrpop2 P_((PCODE *p));
static void foldxref P_((PCODE *p));
static void optjrst P_((PCODE *p));
...
int op, r;
{
    shocum();				/* Force changes before header */
...
PCODE *p;
{
    unsigned int i, ptr=0, val1=0, val2=0;


    /* Indicate code table entry number */
    fprintf(f,"codes[%d]=", idx);
	...
	case PTA_MINDEXED:		/* addr+offset(index) */
	    fprintf(f, "MINDEXED R=%o, Addr=%s+%o(%o)", p->Preg,
		(p->Pptr ? p->Pptr->Sname : ""), p->Poffset, p->Pindex);
	    /* Only print bsize for interesting cases */
	    switch (p->Pop & POF_OPCODE) {
	    case P_SUBBP:
	    case P_PTRCNV:
	    case P_SMOVE:
		val1 = p->Pbsize;
		fprintf(f, " siz %o", val1);
		break;
	    }
	    break;
    ...
    fprintf(f,"\n\t%6o/ %3o %5o %2o %6o %6o %2o %o+%o\n",
	    p, p->Ptype, p->Pop, p->Preg, ptr,
	    p->Poffset, p->Pindex, val1, val2);
}
...
flushcode()
{
    if (mincode < maxcode) {
	if (debpho) {
	    shocum();			/* Keep differences in sync */
	...
	if (debpho) shocum();		/* Show stuff forced out. */
    }
    previous = NULL;
}

...
VREG *to, *from;
{
    ...
    if (debpho) shocum();		/* Keep differences in sync */
...
VREG *reg, *idx;
{
    ...
    r = reg ? vrstoreal(reg, r3) : 0;	/* Allow IBP/JRST 0, */
    s = vrreal(r3);
    vrfree(r3);
    code40(op, r, s, 0);
}
...
codr8(op, r, val)
{
	...
	val = foldstack(val);
	if (val == 0) return;		/* If completely folded, can return */
    }
