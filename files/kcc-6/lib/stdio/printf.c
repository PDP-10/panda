...
static void prfill P_((struct pf_work *wp,char *pref,char *cval,int clen,int precf));
static void prfn P_((struct pf_work *wp));
static void prfc P_((struct pf_work *wp));
static void prfs P_((struct pf_work *wp));
static void prfp P_((struct pf_work *wp));
static void prfd P_((struct pf_work *wp));
static void prfo P_((struct pf_work *wp));
static void prfx P_((struct pf_work *wp));
static void prfu P_((struct pf_work *wp));
static char *pd P_((char *s,int n));
#if !LONGISINT
static char *pld P_((char *s,long n));
#endif
static char *pu P_((char *s,unsigned int n));
#if !LONGISINT
static char *plu P_((char *s,unsigned long n));
#endif
static char *po P_((char *s,unsigned int n));
#if !LONGISINT
static char *plo P_((char *s,unsigned long n));
#endif
static char *px P_((char *s,unsigned int n));
#if !LONGISINT
static char *plx P_((char *s,unsigned long n));
#endif
static char *pxu P_((char *s,unsigned int n));
#if !LONGISINT
static char *plxu P_((char *s,unsigned long n));
#endif
static void prff P_((struct pf_work *wp));
static void prfe P_((struct pf_work *wp));
static void prfg P_((struct pf_work *wp));
static void pflt P_((struct pf_work *wp,int eflg));
static double prnd P_((double d,int prec));
static char *pfi P_((double d,char *s));
static char *pff P_((double d,char *cp,int prec));
static char *pexp P_((int exp,char *cp));
static double frexp10 P_((double d,int *ip));
...
#endif
{
    ...
    va_start(ap, format);		/* Set up pointer to args */
    ret = vsnprintf(s, size, format, ap); /* output to string */
    va_end(ap);
    return ret;
}
...
/* VSNPRINTF - like SNPRINTF but with explicit arg list
*/
int
...
va_list ap;
{
    ...
    f = sopen(s, "w", size);		/* open string for output */
...

typedef void (*faddr)(struct pf_work *); /* FADDR - function addr type */
