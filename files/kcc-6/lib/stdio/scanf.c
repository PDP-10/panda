...
/* cstdio:scanf.c */
static void n_scanf P_((struct scanf_work *sf));
static void duox_scanf P_((struct scanf_work *sf,int signflag,int base));
static void f_scanf P_((struct scanf_work *sf));
static void c_scanf P_((struct scanf_work *sf));
static void s_scanf P_((struct scanf_work *sf));
static void bracket_scanf P_((struct scanf_work *sf));
...

int vfscanf(stream, format, args)
FILE *stream;
CONST char *format;
va_list args;
{
...
struct scanf_work *sf;
{
	...
	if (c < CHAR_SET_SIZE && valid_char[c]) {
	    if (sf->assign) *s++ = c;
	    sf->ccnt++;
	    sf->error = FALSE;			/* We have a non-null string */
	} else {
	    ungetc(c, sf->stream);
	    break;
	}
    ...
    /* If non-null string and assigning results, terminate with NULL */
    if (!sf->error && sf->assign) {
	*s = '\0';
	sf->asscnt++;
    }
}
