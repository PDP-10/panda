...
a monitor PC then control will resume at .JSABT instead of .JSINT.
...
at all, by jumping to .JSABT.  The sole reason for the existence of the
...
int num, *ablock;
{
	if (0) (`sigusys`++, `sigbkm`++, `sigpnm`++, `frktrm`++);
#asm
	SEARCH MONSYM		/* Uppercase so no conflict with monsym() */
	extern $$sect
	intern .jsbeg, .jsend, .jscct, .jsabt, .jsint	/* Exported syms */
	...
	*/
.jsbeg:
	skiple USYS_VAR_ASM_REF(sigusys) ; Are we within USYS call?
	...
	** Must check carefully; @sigbkm cannot change, but @sigpnm
	** may change at any time!  All instructions between the
	** test and the actual JSYS invocation are critical!
	*/
	setcm 6,USYS_VAR_ASM_REF(sigbkm) ; Get block mask complement
	skipn USYS_VAR_ASM_REF(frktrm)	/* Any pending fork termination? */
jsccx1:	 tdne 6,USYS_VAR_ASM_REF(sigpnm) ; Are any unblocked ints pending?
	  jrst .jsabt		; Signals are pending, abort immediately!
jsccx2:	jrst @clsdsp(14)	; CRITICAL: no, go execute JSYS.

	/* This table had better be in the same section as the PSI
	** handler or the AOBJN pointer won't work.
	*/
.jscct:	-njscct,,.+1		/* AOBJN pointer */
	$$SECT,,jsccx1
	$$SECT,,jsccx2
...
j0lose:	dmove 6,1		/* Save ACs 1 and 2 */
	...
	/* Return values */
.jsabt:	skipa 5,[-2]		; -2 Interrupted before JSYS executed
