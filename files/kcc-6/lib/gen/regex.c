...
/* regex.c */
static int store_jump P_((char *from,int opcode,char *to));
static int insert_jump P_((int op,char *from,char *to,char *current_end));
static int bcmp_translate P_((const char *s1,const char *s2,int len,char *translate));
     ...
     struct re_pattern_buffer *bufp;
{
  ...
  register const char *pend = pattern + size;
...
static int
re_match_2a (pbufp, string1, size1, string2, size2, pos, regs)
     struct re_pattern_buffer *pbufp;
     const char *string1, *string2;
     int size1, size2;
     int pos;
     struct re_registers *regs;
{
  ...
  const char *d = string1, *dend = d + size1;
  const char *dx = string2, *dendx = dx + size2;
  register const char *d1, *dend1;
  register int mcnt;
 ...
 /* Failure point stack.  Each place that can handle a failure further down the line
  ...
  const char **stackb = (const char **) alloca (2 * NFAILURES * sizeof (char *));
  const char **stackp = stackb, **stacke = &stackb[2 * NFAILURES];
  ...
  const char *regstart_segend[RE_NREGS];
  ...
  while (1)
	...
	case on_failure_jump:
	  if (stackp == stacke)
	    {
	      const char **stackx = (const char **) alloca (2 * (stacke - stackb) * sizeof (char *));
...
re_match_2 (pbufp, string1, size1, string2, size2, pos, regs)
     ...
     struct re_registers *regs;
{
  int res = re_match_2a(pbufp, string1, size1, string2, size2, pos, regs);
  alloca(0);			/* Free storage from re_match_2a */
  return res;
}
