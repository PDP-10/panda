...
static double one_over_SQRT2 = 1/SQRT2;

static double logpoly(s)
double s;
{
    double z, zt2;
    ...
    z = (s - 1.0) / (s + 1.0);
    zt2 = z * z;
    return (z * _poly4(log_pcoeffs, zt2) / _poly4(log_qcoeffs, zt2));
}
...
double x;
{
    ...
    if ((x < one_over_SQRT2) || (x > SQRT2)) {
	s = SQRT2 * frexp(x, &k); /* Get mantissa and exponent */
	return (k * LN2) - LNSQRT2 + logpoly(s);
    }
