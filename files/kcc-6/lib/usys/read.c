...
static int in_filbuf P_((struct _ufile *uf,int havesome));
static int in_cbuf P_((struct _ufile *uf));
#if SYS_T20+SYS_10X
static int insys P_((struct _ufile *uf,char *buf,int cnt));
static int indev P_((int num,struct _ufile *uf,char *buf,int cnt));
static int in_slow P_((struct _ufile *uf,char *buf,int cnt));
static int intty P_((struct _ufile *uf,char *buf,int cnt));
static int inpty P_((struct _ufile *uf,char *buf,int cnt));
static int in_rdln P_((struct _tty *tp,struct _ufile *uf,char *buf,int cnt));
static int eofp P_((struct _ufile *uf));
#endif	
#if SYS_T10+SYS_CSI+SYS_WTS
static int insys P_((struct _ufile *uf,char *buf,int cnt));
static int inblk P_((struct _ufile *uf,char *buf,int nbytes));
static int in_slow P_((struct _ufile *uf,char *buf,int cnt));
static int inbuf P_((struct _ufile *uf));
static int eofp P_((struct _ufile *uf));
static int intty P_((struct _ufile *uf,char *buf,int cnt));
static int inctm P_((struct _ufile *uf,char *buf,int cnt));
#endif 
#if SYS_WTS	
static int chkedir P_((struct _ufile *uf));
#endif 
#if SYS_ITS
static int insys P_((struct _ufile *uf,char *buf,int cnt));
static int inblock P_((struct _ufile *uf,char *buf,int nbytes));
static int insiot P_((struct _ufile *uf,char *buf,int cnt));
static int inzbuf P_((struct _ufile *uf,char *buf,int cnt));
static void unzbuf P_((struct _ufile *uf,char *buf,int cnt));
static int padcount P_((char *buf,int cnt));
static int inpadded P_((struct _ufile *uf,char *buf,int cnt));
#endif 
#endif 
...
register size_t nbytes;
{
	...
	for (;;) {			/* Loop for interrupt continuation */
	    ...
	    if (n == 0)			/* If error, errno is already set */
		USYS_RET(-1);
	    ...
	    */
	    ...
	    if (uf != USYS_VAR_REF(uffd[fd])) /* Ensure FD still OK */
...
int havesome;			/* Flag saying whether already got input */
{
	...
	if (n == 0) {
	    if ((errno == EWOULDBLOCK) && (havesome || uf->uf_rleft > 0))
		return -1;	/* Return this way if anything read */
	    return 0;		/* Error, errno already set */
	}
	/* Interrupted.
...
char *buf;
{
	...
	*/
	...
	/* Special PTY input devices, may need parity stripped */
	case _DVPTY:
	    return inpty(uf, buf, cnt);
...

/*
 *	Get # of characters waiting to be read
 */
...
long *arg;
{
...
#if SYS_T20+SYS_10X
  {
    ...
    *arg = 0;			/* Assume no input available */
    acs[1] = uf->uf_ch;		/* JFN on stream */
    i = jsys(SIBE, acs);	/* Check input buffer, char avail to AC2 */
    if (i < 0) return -2;	/* Interrupted? */
    if (i == 0) return -1;	/* Failed? */
    if (i == 1) *arg = acs[2];	/* If not empty, copy count */
    if (uf->uf_rleft > 0)	/* If anything buffered */
	*arg += uf->uf_rleft;	/* then include it too. */
    if (!*arg) {		/* If no input yet, try harder */
      acs[1] = uf->uf_ch;
      if (!jsys(GTSTS,acs)
	  || (acs[2] & (GS_EOF|GS_ERR)))
	(*arg)++;			/* Add 1 for exceptional condition */
      else switch (uf->uf_type) {	/* Otherwise, special checks */
      case _DVTTY:		/* For TTY, we have the whole answer */
	break;

      case _DVPTY:		/* PTY, check other end */
	acs[1] = uf->uf_ch;
	if (jsys(DVCHR, acs) <= 0) return 0; /* Get unit in RH AC3 */
	acs[1] = monsym(".TTDES") | ((USYS_VAR_REF(ptytty) + acs[3]) & RH);
	if ((i = jsys(SOBE, acs)) <= 0) return 0; /* TTY output buffer empty */
	if (i == 1) (*arg) += acs[2]; /* Not empty, add count */
	break;

      case _DVPIP:		/* Pipe, use GDSTS */
	acs[1] = uf->uf_ch;	/* Get JFN */
	if (jsys(GDSTS,acs) > 0) {
	  *arg = acs[3] & RH;	/* Get actual count of bytes available */
	  if (acs[2] & 010) (*arg)++; /* If write side closed, add 1 more */
	}
	break;
	...
	if (!*arg) *arg = 1;	/* If no answer yet, say at least 1 */
	break;
      }
    }
...
char *buf;
{
    ...
    n = _nread(uf, &avail);	/* Get characters available */
    if (n == -2) return -cnt;	/* If interrupted, say so */

    if (avail > 0) {		/* Any bytes available? */
	if (cnt < avail)	/* Yes!  Do we want that many? */
	    avail = cnt;	/* No, read only what was requested */
	n = indev((int)(SIN|JSYS_OKINT), uf, buf, avail);
	if (n < 0) return avail + n - cnt;
				/* Compute number unread of those requested */
	return n;		/* Return number read + 1 */
    }

    /* Apparently no characters available */

    /* If non-blocking I/O and TTY/PTY/PIPE, return */
    if (uf->uf_flgs & UIOF_NDELAY)
	switch (uf->uf_type) {
	case _DVTTY:
	case _DVPTY:
	case _DVPIP:
    ...
    acs[1] = uf->uf_ch;
    n = indev((int)(SIN|JSYS_OKINT), uf, buf, 1);
    if (n < 0)			/* Interrupted? */
	return -cnt;		/* Say interrupted, nothing read */

    if (n < 2) return n;	/* Return if error or eof */

    /* Got a byte!  Update things and see if there's any more input. */
    buf++;			/* Advance buffer pointer */
    if (cnt == 1)		/* Only 1 byte requested? */
	return 1 + 1;		/* Yes, return number read + 1 */

    n = _nread(uf, &avail);	/* Get characters available */
    if (n == -2) return -(cnt - 1); /* If interrupted, say so */
				/* By returning -number unread */
    if (avail > 0) {		/* Any more available? */
        if ((cnt - 1) < avail)	/* Yes, do we want that many or more? */
	    avail = cnt - 1;	/* No, just read what was requested */
	n = indev((int)(SIN|JSYS_OKINT), uf, buf, avail);
	if (n < 0) {		/* Interrupted? */
	  n += avail + 1;	/* Yes, compute number actually read */
				/* (Including BIN) */
	  return n - cnt;	/* Return -number unread */
	}
	if (n > 0) return n + 1; /* Add 1 for BIN to number read + 1 */
    }
    return 1 + 1;		/* Return number by BIN + 1 */
}

/* INPTY - system-dependent PTY input routine.
...
char *buf;
{
    ...
    n = in_slow(uf, buf, cnt);
    if ((n < 2) || (uf->uf_bsize > 7))
      return n;	/* If no data or byte size greater than 7, just return */
    mask = (1 << uf->uf_bsize) - 1;
    bn = n;
    while (--bn) *buf++ &= mask; /* Clear possible parity bits */
    return n;
}

...
char *buf;
{
    ...
    tp = &USYS_VAR_REF(ttys[uf->uf_dnum]); /* Get ptr to TTY struct */
...
struct texti {		/* Command block for TEXTI% */
	...
	char *rdbfp;	/* Destination buffer pointer */
	char *rdrty;	/* ^R buffer pointer */
	int *rdbrk;	/* Pointer to break mask block */
};
...
char *buf;
{
	...
	textib = {(sizeof(struct texti)/sizeof(int)) - 1,
		  RD_CRF|RD_JFN, 0, 0, 0, 0, 0, brkmsk};
    int i, acs[5];
    unsigned char bc;
    ...
    textib.rddbp = buf-1;
    textib.rddbc = cnt;
    brkmsk[0] = 000220000000;	/* break on CR & LF */
    brkmsk[1] = brkmsk[2] = brkmsk[3] = 0;
    if ((bc = tp->tc.t_eofc) < 0200)
	brkmsk[bc / 36]	|= 1 << (35 - (bc % 36));	/* EOF char */
    if ((bc = tp->tc.t_brkc) < 0200)
	brkmsk[bc / 36]	|= 1 << (35 - (bc % 36));	/* BRK char */
    acs[1] = (int) &textib;
    ...
    /* Won normally */
    if (*textib.rddbp == tp->tc.t_eofc) {/* Stopped on EOF char? */
...
int cnt;
{
	...
	case 0:		/* Cooked mode */
		...
		if (inchar == tp->tc.t_eofc) {	/* EOF char? */
