...
(defconst make-backup-files (not (or (eq system-type 'vax-vms)
...
for the \"mode:\" local variable.  For that, use  hack-local-variables."
  ...
  (let (beg end mode)
	...
	(let ((alist auto-mode-alist)
	  ...
	  (let ((case-fold-search (or (eq system-type 'vax-vms)
				      (eq system-type 'tops20))))
...
if you wish to pass an empty string as the argument."
  ...
  (if filename
	...
	(if (or (eq system-type 'vax-vms) (eq system-type 'tops20))
...
if variable  delete-auto-save-files  is non-nil."
  (and buffer-auto-save-file-name delete-auto-save-files
       ...
       (progn
	 (condition-case ()
	     (delete-file buffer-auto-save-file-name t)
	   (file-error nil))
